 ...
 * Class IsUppercase
 *
 * This class is a custom attribute used to validate if a property of an object is a string in uppercase.
 * Optionally, it can also validate if each item in an array is a string in uppercase.
 *
...
class IsUppercase implements IValidateConstraint
{
     ...
     * @param bool $each If true, each item in the array should also be a string in uppercase.
     */
     ...
     *
     * Checks if value is a string in uppercase. If $each is true, it checks each item in the array.
     *
     * @param ReflectionProperty $property The property to validate.
     * @param object $object The object containing the property.
     * @return bool Returns true if the property is a string in uppercase and, if $each is true, each item in the array is also a string in uppercase.
     */
    public function validate(ReflectionProperty $property, object $object): bool
    {
        ...
        if ($this->each && is_array($value)) {
            return _Array::every($value, fn($item) => is_string($item) && mb_strtoupper($item) === $item);
        }
    ...
    public function defaultMessage(ReflectionProperty $property, object $object): string
    {
        ...
        $message = "Property {$property->getName()} must be a uppercase string";
